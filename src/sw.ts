...
const STATIC_FILES_URL = (self.__WB_MANIFEST || []).map((pair: any) => {
    return pair.url;
});
 ...
 */
async function networkFirst(request: any, clientId: any) {
 ...
 * @return {Promise<void>}
 */
async function sendMessage(clientId: any, messageType: any, url: any) {
    // @ts-expect-error ts-migrate(2552) FIXME: Cannot find name 'clients'. Did you mean 'client'?
