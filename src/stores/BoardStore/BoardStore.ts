...
// Actions
import {BoardsActionTypes} from '../../actions/boards';
...
import {InviteActionTypes} from '../../actions/invite';
...
class BoardStore extends BaseStore {
     ...
     * @private
     */
    _setBoardInvite(accessPath: any) {
     ...
     * @private
     */
    _getTagById(tgid: any) {
        return this._storage.get('tags-list-popup').tags.find((tag: any) => {
     ...
     * @private
     */
    _getTagColorById(clrid: any) {
        return this._storage.get('tag-popup').colors.find((color: any) => {
    ...
    async _getBoard(data: any) {
        ...
        case HttpStatusCodes.Ok:
            ...
            this._storage.get('tags-list-popup').tags = payload.data.tags.map((tag: any) => {
                tag.selected = false;
                return tag;
            });

            this._storage.get('tag-popup').colors = payload.data.colors.map((color: any) => {
                color.selected = false;
                return color;
            });

            this._storage.get('card_lists').forEach((cardlist: any) => {
                cardlist.cards.forEach((card: any) => {
     ...
     * @private
     */
    async _updateTitleAndDescription(data: any) {
        ...
        case HttpStatusCodes.Ok:
            ...
            this._storage.set('board_name', payload.data.board_name);
            this._storage.set('description', payload.data.description);
            UserStore.ws.send(this._storage.get('bid'));
            return;
     ...
     * @private
     */
    async _processHideConfirmDialog(data: any) {
        this._storage.get('setting-popup').errors = null;
        this._storage.get('setting-popup').confirm = false;
        if (!data.confirmed) {
            return;
     ...
     * @private
     */
    _getCardListById(clid: any) {
        return this._storage.get('card_lists').find((cardList: any) => {
     ...
     * @private
     */
    _showEditCardListPopUp(data: any) {
     ...
     * @private
     */
    async _updateCardList(data: any) {
        if (SettingsStore.isOffline()) {
            return;
        }
        this._storage.get('cardlist-popup').errors = null;
        ...
        case HttpStatusCodes.Ok:
            ...
            cardLists.sort((lhs: any, rhs: any) => {
     ...
     * @private
     */
    _showDeleteCardListPopUp(data: any) {
     ...
     * @private
     */
    async _deleteCardList(data: any) {
        this._hideDeleteCardListPopUp();
        if (!data.confirm) {
            return;
        }
        ...
        case HttpStatusCodes.Ok:
            ...
            const cardLists = this._storage.get('card_lists');
            const clid = cardLists.findIndex(({
                clid,
            }: any) => clid === clidForDeletion);
            if (clid === -1) {
     ...
     * @private
     */
    _getCardById(clid: any, cid: any) {
        const result = this._getCardListById(clid).cards.find((card: any) => {
            return card.cid === cid;
        });
        if (!result) {
    ...
    _showEditCardPopUp(data: any) {
        ...
        this._storage.set('card-popup', {
            ...
            errors: null,
            tags: card.tags,
            checkLists: this._getCardById(data.clid, data.cid).check_lists.map((list: any) => {
                const items = list.check_list_items.map((item: any) => {
     ...
     * @private
     */
    async _updateDeadlineCheck(data: any) {
        if (SettingsStore.isOffline()) {
            return;
        }
    ...
    async _deleteCard(data: any) {
        ...
        case HttpStatusCodes.Ok:
            ...
            const cid = cardLists[clid].cards.findIndex(({
                cid,
            }: any) => cid === cidForDeletion);
            if (cid === -1) {
     ...
     * @private
     */
    _getCheckListById(chlid: any) {
        const context = this._storage.get('card-popup');
        return context.checkLists.find((list: any) => {
     ...
     * @private
     */
    _getCheckListItemById(chlid: any, chliid: any) {
        return this._getCheckListById(chlid).check_list_items.find((item: any) => {
    ...
    async _createCheckList() {
        ...
        case HttpStatusCodes.Ok:
            UserStore.ws.send(this._storage.get('bid'));
            // @ts-expect-error ts-migrate(2339) FIXME: Property 'chlid' does not exist on type '{ cid: an... Remove this comment to see the full error message
    ...
    async _saveCheckList(data: any) {
        ...
        case HttpStatusCodes.Ok:
            ...
            this._getCardById(context.clid, context.cid).check_lists.find((checkLst: any) => {
     ...
     * @private
     */
    async _deleteCheckList(data: any) {
        const context = this._storage.get('card-popup');
        context.errors = null;
        ...
        case HttpStatusCodes.Ok:
            UserStore.ws.send(this._storage.get('bid'));
            let delCheckList = context.checkLists.find((checklist: any) => {
                return checklist.chlid === data.chlid;
            });
            context.checkLists.splice(context.checkLists.indexOf(delCheckList), 1);
            delCheckList = this._getCardById(context.clid, context.cid).check_lists.find((checkLst: any) => {
                return checkLst.chlid === data.chlid;
            });
     ...
     * @private
     */
    async _createCheckListItem(data: any) {
        if (SettingsStore.isOffline()) {
            return;
        }
        const context = this._storage.get('card-popup');
        ...
        case HttpStatusCodes.Ok:
            UserStore.ws.send(this._storage.get('bid'));
            // @ts-expect-error ts-migrate(2551) FIXME: Property 'chliid' does not exist on type '{ chlid:... Remove this comment to see the full error message
     ...
     * @private
     */
    async _saveCheckListItem(data: any) {
        if (SettingsStore.isOffline()) {
            return;
        }
        const context = this._storage.get('card-popup');
        ...
        case HttpStatusCodes.Ok:
            ...
            this._getCardById(context.clid, context.cid).check_lists.find((checkLst: any) => {
                return checkLst.chlid === data.chlid;
            }).check_list_items.find((chLstItem: any) => {
     ...
     * @private
     */
    async _deleteCheckListItem(data: any) {
        const context = this._storage.get('card-popup');
        context.errors = null;
        ...
        case HttpStatusCodes.Ok:
            ...
            list = this._getCardById(context.clid, context.cid).check_lists.find((checkLst: any) => {
                return checkLst.chlid === data.chlid;
            });
            item = list.check_list_items.find((checkLstItem: any) => {
     ...
     * @private
     */
    async _toggleCheckListItem(data: any) {
        if (SettingsStore.isOffline()) {
            return;
        }
        const context = this._storage.get('card-popup');
        ...
        case HttpStatusCodes.Ok:
            ...
            item = this._getCardById(context.clid, context.cid).check_lists.find((checkLst: any) => {
                return checkLst.chlid === data.chlid;
            }).check_list_items.find((checkLstItem: any) => {
                return checkLstItem.chliid === data.chliid;
            }).status = !item.status;
            return;
    ...
    _showAddCardAssigneePopUp() {
        ...
        context.users = card.assignees.map((assignee: any) => {
     ...
     * @private
     */
    async _refreshCardAssigneeSearchList(data: any) {
        const context = this._storage.get('add-card-member-popup');
        context.selectInvite = false;
        context.errors = null;
     ...
     * @private
     */
    async _toggleCardAssigneeInSearchList(data: any) {
        if (SettingsStore.isOffline()) {
            return;
        }
        const context = this._storage.get('add-card-member-popup');
    ...
    _showAddBoardMemberPopUp() {
        ...
        context.errors = null;
        context.searchString = null;
        context.users = this._storage.get('invited_members').map((member: any) => {
     ...
     * @private
     */
    _removeUserFromCards(uid: any) {
        this._storage.get('card_lists').forEach((cardList: any) => {
            cardList.cards.forEach((card: any) => {
                const cardMember = card.assignees.find((assignee: any) => {
     ...
     * @private
     */
    async _toggleBoardMemberInSearchList(data: any) {
        if (SettingsStore.isOffline()) {
            return;
        }
        const context = this._storage.get('add-board-member-popup');
        ...
        const member = members.find((memeber: any) => {
     ...
     * @private
     */
    async _refreshBoardMemberSearchList(data: any) {
        const context = this._storage.get('add-board-member-popup');
        context.selectInvite = false;
        context.errors = null;
        const {searchString} = data;
     ...
     * @private
     */
    async _createCardComment(data: any) {
        if (SettingsStore.isOffline()) {
            return;
        }
        const context = this._storage.get('card-popup');
     ...
     * @private
     */
    async _editCardComment(data: any) {
     ...
     * @private
     */
    async _updateCardComment(data: any) {
        if (SettingsStore.isOffline()) {
            return;
        }
        let payload;
        ...
        case HttpStatusCodes.Ok:
            UserStore.ws.send(this._storage.get('bid'));
            const comments = this._storage.get('card-popup').comments;
            const comment = comments.find((item: any) => {
     ...
     * @private
     */
    async _deleteCardComment(data: any) {
        let payload;
        ...
        case HttpStatusCodes.Ok:
            ...
            const cmid = card.comments.findIndex(({
                cmid,
            }: any) => cmid === data.cmid);
            if (cmid === -1) {
                throw new Error(`BoardStore: комментарий ${data.cmid} не найден.`);
     ...
     * @private
     */
    _changeCardPopUpScroll(data: any) {
     ...
     * @private
     */
    async _uploadAttachment(data: any) {
        if (SettingsStore.isOffline()) {
            return;
        }
     ...
     * @private
     */
    async _deleteAttachment(data: any) {
        if (SettingsStore.isOffline()) {
            return;
        }
        const cardContext = this._storage.get('card-popup');
        ...
        case HttpStatusCodes.Ok:
            UserStore.ws.send(this._storage.get('bid'));
            const attachment = cardContext.attachments.find((attach: any) => {
     ...
     * @return {Promise<void>}
     */
    async _openBoardInvite(data: any) {
        let payload;
     ...
     * @return {Promise<void>}
     */
    async _openCardInvite(data: any) {
        let payload;
    ...
    _showTagListPopUpCard() {
        ...
        context.tags.forEach((tag: any) => {
            tag.selected = !!currentCard.tags.find((cardTag: any) => {
    ...
    _showTagEditPopUp(data: any) {
        ...
        context.colors.forEach((color: any) => {
    ...
    async _toggleTag(data: any) {
        ...
        case HttpStatusCodes.Ok:
            UserStore.ws.send(this._storage.get('bid'));
            const tagPopUp = context.tags.find((tag: any) => {
            ...
            if (tagPopUp.selected) {
                ...
                currentCard.tags.splice(currentCard.tags.indexOf(currentCard.tags.find((tag: any) => {
                    return tag.tgid === data.tgid;
                })), 1);
            }
