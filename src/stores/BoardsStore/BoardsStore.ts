...
import {BoardStoreConstants, ConstantMessages, HttpStatusCodes} from '../../constants/constants';
...
class BoardsStore extends BaseStore {
    ...
    constructor() {
        ...
        // @ts-expect-error ts-migrate(2339) FIXME: Property '_channel' does not exist on type 'Boards... Remove this comment to see the full error message
        this._channel = 'Boards';

        // @ts-expect-error ts-migrate(2339) FIXME: Property '_storage' does not exist on type 'Boards... Remove this comment to see the full error message
    ...
    async _get() {
        ...
        case HttpStatusCodes.Ok:
            ...
            this._storage.set('teams', payload.data.sort(
                (first: any, second: any) => (first.id > second.id) ?
    ...
    async _create(data: any) {
        ...
        case HttpStatusCodes.Ok:
            // @ts-expect-error ts-migrate(2339) FIXME: Property '_storage' does not exist on type 'Boards... Remove this comment to see the full error message
            const team = this._storage.get('teams').find((team: any) => {
     ...
     * @private
     */
    _showAddTeamMemberPopUp(data: any) {
        // @ts-expect-error ts-migrate(2339) FIXME: Property '_storage' does not exist on type 'Boards... Remove this comment to see the full error message
        ...
        // @ts-expect-error ts-migrate(2339) FIXME: Property '_storage' does not exist on type 'Boards... Remove this comment to see the full error message
        context.users = this._storage.get('teams').find((team: any) => {
            return team.tid === data.tid;
     ...
     * @private
     */
    async _refreshTeamMemberSearchList(data: any) {
        // @ts-expect-error ts-migrate(2339) FIXME: Property '_storage' does not exist on type 'Boards... Remove this comment to see the full error message
        const context = this._storage.get('add-team-member-popup');
        context.errors = null;
     ...
     * @private
     */
    async _toggleTeamMemberInSearchList(data: any) {
        // @ts-expect-error ts-migrate(2339) FIXME: Property '_storage' does not exist on type 'Boards... Remove this comment to see the full error message
        const context = this._storage.get('add-team-member-popup');
        context.errors = null;
     ...
     * @private
     */
    async _submitAddTeamPopUp(data: any) {
        if (SettingsStore.isOffline()) {
            return;
        }
        const validator = new Validator();
     ...
     * @private
     */
    async _submitEditTeamPopUp(data: any) {
        console.log('submit');
        // @ts-expect-error ts-migrate(2339) FIXME: Property '_storage' does not exist on type 'Boards... Remove this comment to see the full error message
        const context = this._storage.get('team-popup');
        const validator = new Validator();
